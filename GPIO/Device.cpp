...
void Device::WorkerProc(void) {
	...
	while (!terminate) {
		...
		int bytes = recv(socketHandle, &buffer, 1, 0);
	...
	while (!terminate) {
		if (connectRequest) {
			...
			if (connected) {
				...
				if (waitForReply >= 0) {
					waitForReply++;
					if (waitForReply == 3000/HANDLE_PERIOD) {
				...
				if (keepAlivePeriod > 0 && ((++kaCounter) >= (keepAlivePeriod/HANDLE_PERIOD)) ) {
					...
					if (SendBuf(keepAliveBuf)) {
						waitForReply = -1;
					}
				} else {
					// handle command queue
					ScopedLock<Mutex> lock(mutexQueue);
					if (bufQueue.empty() == false) {
						std::string buf = bufQueue.front();
						bufQueue.pop_front();
						SendBuf(buf);
						kaCounter = 0;
					}
...
void Device::ResetWaitForReply(void) {
	waitForReply = -1;
}
